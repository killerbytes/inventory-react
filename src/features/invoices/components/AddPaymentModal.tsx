...
import { ApiErrorResponse, Invoice, paymentBaseSchema } from "@/schemas";
...
export default function AddPaymentModal({
  ...
  const onSubmit = async (values: z.infer<typeof paymentBaseSchema>) => {
    const payload = {
      ...values,
      applications: [
        {
        ...
        onError: (error: unknown) => {
          ...
          if (apiError.code === ERROR.VALIDATION_ERROR) {
            apiError.errors?.forEach((err) => {
              if (err.field) {
                form.setError(
                  err.field as keyof z.infer<typeof paymentBaseSchema>,
                  {
                    type: "server",
                    message: err.message,
                  },
